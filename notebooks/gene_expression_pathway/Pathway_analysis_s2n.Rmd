---
title: "Pathway analysis signal to noise"
output: 
  html_document:
    toc: yes
    toc_float: yes
...
signal2noise = DE_results %>% 
  dplyr::select(cell,batch,results) %>% 
  unnest() %>% 
  separate(gene_id,c("ensembl","symbol"),sep = "\\_") %>% 
  dplyr::select(cell,batch,symbol,stat) %>% 
  mutate(
    batch = factor(batch,levels = c("old","new"))
  ) 
...
pathways = pathways %>% 
  ...
  mutate(
    fc = map(fc,inner_join,signal2noise,by = "symbol"),
    new_batch_summary = map(fc,filter,batch == "new") %>% 
      map(dplyr::select,-batch) %>% 
      map(spread,cell,stat),
...
pathways = pathways %>% 
  mutate(
    plots = map2(fc,ID, 
                 ~ .x %>% 
                   ggplot(aes(reorder(symbol,-stat),
                              stat,colour = cell,shape = batch))+
                   geom_point(size = 2.5)+
                   theme(
