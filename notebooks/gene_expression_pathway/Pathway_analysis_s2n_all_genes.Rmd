---
title: "Pathway analysis signal to noise (with all genes)"
output: 
  html_document:
    toc: yes
    toc_float: yes
...
DE_results = DE_results %>% 
  mutate(
    gene_list = map(filtered_results, ~ arrange(.,desc(stat))) %>% 
      map(separate,gene_id,into = c("ensembl","symbol"),sep ="\\_",extra = "merge") %>% 
      map(name)
  ) %>% 
  mutate(
...
```{r gsea_hallmark_plots,include=TRUE,fig.width=9,fig.height=16}
