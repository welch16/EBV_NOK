...
rm(diff_genes_EBV,diff_genes_NOKS)
...
I am going to focus on __a)__ as it seems more conservative and try the mean ranking, as the median appears to shrink everything and the quantile seems to behave similarly to the mean. 
...
```{r volcano_plot,fig.height=3,fig.width=6.5,fig.align="center"}
fdr = 1e-2
diff_genes = diff_genes %>% 
  mutate(
    results = map(results,mutate,padj = p.adjust(pvalue,method = "fdr"))) %>% 
  unnest() %>% 
  filter(MC <= K & none <= K) %>% 
  mutate(
...
```{r gsea_full_plots,include=TRUE,fig.width=9,fig.height=6}
...
```{r gsea_diff_plots,include=TRUE,fig.width=9,fig.height=6}
plot_data = gsea %>%
...
## Further exploration of pathways obtained with DE genes

A lot of the pathways coincide, hence I am going to focus on the ones obtained when using the DE genes explicitly.

```{r explo,include=TRUE,fig.height=4,fig.width=12,echo=FALSE,warning=FALSE,message=FALSE,results='hide'}
