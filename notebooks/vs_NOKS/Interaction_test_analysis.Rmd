...
diff_genes %<>%
  ...
  mutate_at(vars(contains("genes_list")),
            funs(map(.,GSEA,TERM2GENE = hallmark,verbose = FALSE) )) %>% 
  mutate_at(vars(contains("genes_list")),
            funs(  map(. , "result") %>% 
                    map(as_tibble)))
...
nes_plot <- function(gsea_data,gene_list  = c("all_genes_list","diff_genes_list"),
                     treatment = "none")
{
...
plots_all_genes = nes_plot(diff_genes,
                            gene_list = "all_genes_list",treatment = "")
plots_diff_genes = nes_plot(diff_genes,
...
## Differentially expressed genes (defined by padj $\leq$ 0.05 )
