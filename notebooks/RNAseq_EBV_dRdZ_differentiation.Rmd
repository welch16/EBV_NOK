...
DE_effects = geneSets[-1] %>% 
  map(effect_plot,
      base_data,median)
UP_effects = geneSets[-1] %>% 
  map(effect_plot,
      base_data %>% 
        filter(log2FoldChange > 0),
      median)
DO_effects = geneSets[-1] %>% 
  map(effect_plot,
      base_data %>% 
        filter(log2FoldChange < 0),
      median)
...
## get legend 
gg = ggplotGrob(DE_effects[[1]] + 
                  theme(legend.position = "top"))$grobs
...
grobs = EBV_subsets %>% 
  map(.f = function(x){
    arrangeGrob(
      DE_effects[[x]] + ylab("Median TPM") + ggtitle("All Diff. Expre. genes"),
      UP_effects[[x]] + ylab("Median TPM") + ggtitle("All UpReg. genes"),
      DO_effects[[x]] + ylab("Median TPM") + ggtitle("All DownReg genes"),
      nrow = 1)
  })
...
In the plot above:
  ...
  * First row, genes diff. expressed in all three cell lines
   
  * Second row, genes diff. expressed in EBV dRdZ and EBV wt but not NOKS.
  
  * Third row, genes diff. expressed in EBV dRdZ only.
...
fit_linear_models <- function(subsets,venn,res1_mat,res2_mat,common_mat,col)
{
  ...
  ## did test by hand and models with log are better    
  # models = list()
  # models[["Z"]] = lm(log(1 + Z) ~ log(1 + X) , data = data)
  # models[["Y"]] = lm(log(1 + Y) ~ log(1 + X) , data = data)
...
test1 = fit_linear_models(c("111","100","001"),
                              venn,
