...
## countMats = diffgenes %>% map(.f = function(x){
##     dipmat %>% dplyr::filter(ensembl %in% x$ensembl) %>%
##         select(-contains("rep0"))})
...
## mC_countData_list = countMats %>% map(transform2matrix,"mC")
## hmC_countData_list = countMats %>% map(transform2matrix,"hmC")
...
## ## example with Scott MC genes

## ## MC methylation

## deseqDataMC = deseqInput(mC_countData_list[[4]])
## deseqDataHMC = deseqInput(hmC_countData_list[[4]])


## deseqMC = DESeq(deseqDataMC,minReplicatesForReplace = Inf)
## deseqHMC = DESeq(deseqDataHMC,minReplicatesForReplace = Inf)

## resultsMC = results(deseqMC,cooksCutoff = FALSE,
##                     contrast = c("cell","EBV","NOKS"),tidy =TRUE,
##                     parallel = TRUE,BPPARAM = bp) %>% as.tbl
## resultsHMC = results(deseqHMC,cooksCutoff = FALSE,
##                     contrast = c("cell","EBV","NOKS"),tidy =TRUE,
##                     parallel = TRUE,BPPARAM = bp) %>% as.tbl

## rldMC = rlog(deseqMC,blind = FALSE)
## rldHMC = rlog(deseqHMC,blind = FALSE)
...
## figsdr = "figs/diff_methylation/heatmaps"



## heat_wrap <- function(rld,K,pref)
## {
##     coldata = colData(rld) %>% as.data.frame
##     coldata$sizeFactor = NULL
...
## ## colors = list(
## ##     cell = brewer.pal(8,"Set1")[1:2])

## ## names(colors)[[1]] = c("EBV","NOKS")

##     topgenes = head(order(rowVars(assay(rld)),decreasing = TRUE),K)
##     MM = mat[topgenes,]
##     pheatmap(MM,color = r,annotation_col = coldata,
##              show_rownames = FALSE,
##              filename = file.path(figsdr,
##                                   paste0(pref,"_heatmap_top_",K,"_genes.pdf")))
## }

## heat_wrap(rldMC,10,"Scott_worep1_MC")
## heat_wrap(rldMC,25,"Scott_worep1_MC")
## heat_wrap(rldMC,100,"Scott_worep1_MC")
## heat_wrap(rldMC,500,"Scott_worep1_MC")
## heat_wrap(rldMC,1000,"Scott_worep1_MC")
## heat_wrap(rldMC,5000,"Scott_worep1_MC")
## heat_wrap(rldMC,1e4,"Scott_worep1_MC")

## heat_wrap(rldHMC,10,"Scott_worep1_HMC")
## heat_wrap(rldHMC,25,"Scott_worep1_HMC")
## heat_wrap(rldHMC,100,"Scott_worep1_HMC")
## heat_wrap(rldHMC,500,"Scott_worep1_HMC")
## heat_wrap(rldHMC,1000,"Scott_worep1_HMC")
## heat_wrap(rldHMC,5000,"Scott_worep1_HMC")
## heat_wrap(rldHMC,1e4,"Scott_worep1_HMC")
