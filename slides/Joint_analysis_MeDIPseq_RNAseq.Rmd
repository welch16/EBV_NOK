...
```{r example,include = TRUE,eval = TRUE}
  ...
  geneList = mC_jointAnalysis_list[[3]] %>%
    dplyr::rename(log2FC.exp = log2FC,
                  log2FC.methyl = log2FoldChange) %>%
          mutate(log2FC.exp = -log2FC.exp)
  threshold = c(0.001,0.01,0.05,0.1,.3)
  tibble(threshold , 
         ngenesDiffM = threshold %>% map_dbl(.f = function(x) geneList %>% filter(padj.methyl <= x) %>% nrow),
         ngenesDiffE = threshold %>% map_dbl(.f = function(x)geneList %>% filter(padj.exp <= x ) %>% nrow),
         ngenesInt = threshold %>% map_dbl(.f = function(x) geneList %>% 
...
```{r process_tpm_dip_matrices,include = FALSE,echo = FALSE,eval = TRUE}
...
```{r explorPlots , include = FALSE,echo = FALSE,eval = TRUE}
  ...
  tt = geneList %>% filter(padj.exp <= 0.1 & padj.methyl <= .1)

  base = tt %>% ggplot(aes(x = log2FC.exp,y = log2FC.methyl)) + geom_point(size = 3) +
    xlab("log2FC expression")+
    ylab("log2FC methylation")+
    geom_abline(slope = 0 , intercept = 0 , linetype = 2 ,colour = "red") +
    geom_vline(xintercept = 0,linetype = 2 ,colour = "red")+theme_minimal()+coord_fixed()+
    ylim(-2,2)

  withlabels0 = base +geom_text_repel(aes(label = gene),show.legend = FALSE)
  ...
  withlabels = withlabels0 + xlim(-2,2)
...
```{r img0,include = TRUE,eval = TRUE,warning=FALSE,fig.height = 5}
...
```{r img01,include = TRUE,eval = TRUE,warning=FALSE,fig.height = 5}
withlabels
```
...
```{r img1, include = TRUE,eval = TRUE,warning = FALSE,fig.height=5}
...
```{r quad1_1,include = TRUE,eval = TRUE,fig.height=5}
...
```{r quad1_2,include = TRUE,eval = TRUE,fig.height=5}
...
```{r quad2_1,include = TRUE,eval = TRUE,fig.height=5}
...
```{r quad2_2,include = TRUE,eval = TRUE,fig.height=5}
...
```{r quad2_3,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("TRAP1",tpmmat,dipmat)

```

## Genes in the red zone

```{r imag2 , include = TRUE,eval = TRUE,warning = FALSE, fig.height=5}
...
```{r quad3_1,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("SLC25A17",tpmmat,dipmat)

```

## Genes in the red zone

```{r quad3_2,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("BBC3",tpmmat,dipmat)

```

## Genes in the red zone

```{r quad3_3,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("AEN",tpmmat,dipmat)
...
```{r quad4_1,include = TRUE,eval = TRUE,fig.height=5}
...
```{r quad4_2,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("ZNF790",tpmmat,dipmat)

```

## Genes in the red zone

```{r quad4_3,include = TRUE,eval = TRUE,fig.height=5}

gene_plot("PLA2G16",tpmmat,dipmat)
